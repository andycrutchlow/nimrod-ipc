...
/**
 * There should never be more than 2 entries in a queue...the one being worked on and the latest arrived.
 * Every message for same subject gets process sequentially by same thread in the event there is a backlog...
 * or delegate to a new thread if there is no current thread for the subject.
 * @author andy
 *
 */
public class ConflatingQueueExecutor  extends QueueExecutor
{
	@Override
	...
	{
		//This is the conflating flavor so as a result of this there can only be 1 or 2 entries i.e. if there
		//is one already the

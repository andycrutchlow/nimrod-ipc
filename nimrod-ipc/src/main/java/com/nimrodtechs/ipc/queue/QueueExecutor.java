 ...
 * Note : this means messages arriving as result of wildcard subscriptions will be handled by same thread which means the
 * listener needs to handle splitting of concurrent work and queue logic.
 * @author andy
 *
 */
public abstract class QueueExecutor implements UncaughtExceptionHandler
{
    ...
    public void setThreadNamePrefix(String threadNamePrefix) {
        if(this instanceof ConflatingQueueExecutor)
    ...
    //Different ways of assigning this value..start with a default which can be override thru commandline vm args
    private int threadPoolSize = System.getProperty("subscriptionThreadPoolSize") != null ? 
            Integer.parseInt(System.getProperty("subscriptionThreadPoolSize")) : (Runtime.getRuntime().availableProcessors() / 2);
    ...
    class TF implements ThreadFactory
    {
     ...
     /**
     * More lightweight/dynamic version that will increase/decrease threadpool based on usage.
     * @throws SystemException
     */
    public void initialize() 
    {
        //serviceThreads = (ThreadPoolExecutor) Executors.newCachedThreadPool(new TF(this));
    ...
    {
        if(mpe.queueExecutor instanceof SequentialQueueExecutor)
        {
...
//          }
        ...
        {
            //Conflating queue - so flip between 2 slots ... when both are empty then we are done
     ...
     * period of time get processed in order by the same thread if there is a backlog. A later message
     * cannot over take an earlier one no matter how long an messageReceived
     * takes.
     * 
     * @author andy
     * 
    ...
    protected final class ServiceMessageTask implements Runnable
    {
        ...
        /**
         * This thread is dedicated to processing all messages for a particular subject for a period of time
         * whilst there are current messages to be consumed for the subject
         */
        @Override
        public void run()
        {
                ...
                if ( list != null )
                {
                    ...
                    for ( int i = 0; i < size; i++ )
                    {
                        try
                        {
                            ...
                            //Only deserialize once ...don't deserialize at all if the payload class is byte[]
    ...
    {
        //Message just consists of a 8 byte time stamp and a serialized data class
        //TODO Do something useful with the timestamp ... update statistics on this subscriber transport, subject (needs to be passed in), message size etc...
        long timestamp = extractLong(aMessage,0,8);
        //The payloadClass indicates the desired class to deserialize and return..if its a byte[] then keep intact 

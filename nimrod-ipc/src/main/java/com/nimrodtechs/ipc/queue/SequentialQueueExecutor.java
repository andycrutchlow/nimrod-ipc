...
/**
 * Simple FIFO queue handling...new messages get added to end of queue..every message for same subject
 * gets process sequentially by same thread in the event there is a backlog...
 * or delegate to a new thread if there is no current thread for the subject.
 * Note : subject is the unit of workflow control. Actual Subject is the actual subject of message.
 * E.g. subject might be xs.rate* i.e. a wildcard, actual subject might be xs.rate.EUR.USD.SPOT or 
 * @author andy
 *
 */
public class SequentialQueueExecutor extends QueueExecutor
{
	@Override
